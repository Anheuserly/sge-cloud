...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await normalizePhones({ dryRun: true, limit: parseLimit(searchParams.get('limit')) });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = await normalizePhones({ dryRun, limit: parseLimit(body.limit) });
    return NextResponse.json({ success: true, ...data });
  } catch (error: any) {
