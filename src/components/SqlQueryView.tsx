...
export const SqlQueryView: React.FC<SqlQueryViewProps> = ({
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto flex flex-col h-[calc(100vh-4rem)]">
        ...
        <div className="flex items-center space-x-2">
          {sampleQueries.map((q, idx) => (
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-neutral-300 text-[11px] font-medium border border-[#333]/60"
            >
              {q.label}
            </button>
          ))}
      ...
      {/* Editor Box */}
      <div className="bg-[#000000] border border-[#222] shadow-sm rounded-lg border border-[#222] p-4 space-y-3 shrink-0">
        <textarea
          rows={5}
          value={sql}
          ...
          className="w-full glass-input p-3 rounded-lg text-xs font-mono placeholder:text-slate-600 focus:ring-1 focus:ring-purple-500 text-purple-200"
        />
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-5 py-2 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-semibold shadow-lg shadow-purple-500/20 disabled:opacity-50 transition-all"
          >
      ...
      {/* Error display */}
      {error && (
        <div className="p-4 rounded-lg bg-rose-950/90 border border-rose-800 text-rose-200 text-xs flex items-start space-x-3 shrink-0">
      ...
      {/* Query Results Table */}
      <div className="flex-1 bg-[#000000] border border-[#222] shadow-sm rounded-lg border border-[#222] flex flex-col overflow-hidden">
        <div className="p-3 px-4 border-b border-[#222] bg-slate-900/90 flex items-center justify-between">
          <span className="text-xs font-semibold text-neutral-300 flex items-center space-x-2">
          ...
          {results && results.length > 0 && (
            <div className="flex items-center space-x-2">
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-neutral-300 text-xs flex items-center space-x-1"
              >
              ...
              <thead className="bg-slate-900/90 text-neutral-400 font-semibold tracking-wider uppercase sticky top-0 z-10 border-b border-[#222] backdrop-blur-md">
                <tr>
                  {Object.keys(results[0]).map((key) => (
                    <th key={key} className="px-4 py-3 border-r border-[#222]/40">
                      {key}
                    </th>
                  ))}
                </tr>
                ...
                {results.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-900/80 transition-colors">
                    {Object.keys(results[0]).map((key) => {
                      ...
                      return (
                        <td key={key} className="px-4 py-2.5 max-w-xs truncate border-r border-[#222]/30">
                          {isNull ? (
